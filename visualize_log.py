...
for logfile in logfiles:
    with open(logfile) as f:
        lines = [line.split("\t") for line in f.read().split("\n") if line and line[:4] != "TEST"]
        xs = [float(line[1]) for line in lines]
        losses = [float(line[2]) for line in lines]
        perps = [float(line[3]) for line in lines]
...
for n, title in enumerate(log_data):
    ...
    x_pos = log_data[title]["xs"][0] + 7000
    y_pos = log_data[title]["ys"][0] - 50
